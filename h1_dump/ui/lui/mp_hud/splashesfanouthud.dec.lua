...
f0_local0 = Engine.UsingSplitscreenUpscaling()
f0_local1 = 0
f0_local2 = 1
local f0_local3 = 2
local f0_local4 = 3
...
function GetChallengeSplashData( f5_arg0, f5_arg1, f5_arg2 )
	...
	local f5_local2 = Engine.TableLookup( SplashTable.File, 0, f5_local0, SplashTable.Cols.Image )
	if not f5_local2 or f5_local2 == "" then
		f5_local2 = "ui_challenge_unlocked"
	end
	return f5_local1, f5_local2, GetChallengeNameWithTier( f5_local0, f5_arg1 ), Engine.Localize( Engine.TableLookup( SplashTable.File, 0, f5_local0, SplashTable.Cols.Desc ), f5_arg2 ), f5_local0
end
...
function handleMedalSplash( f6_arg0, f6_arg1 )
	...
	if HasSplashData( f6_arg1.splash ) then
		...
		DebugPrint( "Could not find splashIdx (" .. splashIdx .. ")" )
	end
end
...
function handleRankUpSplash( f8_arg0, f8_arg1 )
	...
	if HasSplashData( f8_local0 ) then
		...
		DebugPrint( "Could not find splashIdx (" .. f8_local0 .. ")" )
	end
end
...
function handleKillstreakSplash( f9_arg0, f9_arg1 )
	...
	if HasSplashData( f9_arg1.splash ) then
		...
		OnKillstreakSplash( f9_arg0, f9_local1, f9_local2, f9_local3, nil, f9_local5, f9_arg1.controller, f9_arg1.count )
		f9_local0 = true
	else
		DebugPrint( "Could not find splashIdx (" .. splashIdx .. ")" )
	end
...
function OnKillstreakSplash( f20_arg0, f20_arg1, f20_arg2, f20_arg3, f20_arg4, f20_arg5, f20_arg6, f20_arg7 )
	if not f20_arg2 or not f20_arg1 then
		return 
	end
	...
	f20_local0.frame:cancelAnimateToState()
	...
	f20_local0.medal:registerAnimationState( "correctIcon", {
		alpha = 1,
		material = RegisterMaterial( f20_arg2 ),
		scale = 2
	} )
	f20_local1 = MBh.AnimateSequence( {
		{
	...
	table.insert( f20_local1, {
		"default",
		f20_arg1 - f0_local14 - f0_local15
	} )
	...
	f20_local0.title:setText( f20_arg3 )
...
function medalSplash( f24_arg0 )
	...
	self:registerAnimationState( "1", f24_local3 )
	self:registerAnimationState( "2", f24_local4 )
	self:registerAnimationState( "3", f24_local5 )
	self:registerAnimationState( "4", f24_local6 )
	self:registerAnimationState( "5", f24_local7 )
	self:registerAnimationState( "6", f24_local2 )
	self.id = "locationAnchor"
	self.locationAnchor = self
	self:addElement( self )
	self:animateToState( "1" )
...
function createKillStreakSplash( f25_arg0 )
	...
	self.id = "killStreakContainer_" .. f25_arg0
	...
	local self = LUI.UIElement.new( f25_local3 )
	self:registerAnimationState( "1", f25_local4 )
	self:registerAnimationState( "2", f25_local3 )
	self.id = "locationAnchor"
	self.locationAnchor = self
	self:addElement( self )
	self:animateToState( "1" )
	local self = LUI.UIImage.new( {
		material = RegisterMaterial( "h1_ks_splash_backing" ),
		topAnchor = true,
		bottomAnchor = false,
		leftAnchor = false,
		rightAnchor = false,
	...
	self:registerAnimationState( "zoom", {
		alpha = 0,
		scale = 0.6
	} )
	self:registerAnimationState( "zoomin", {
		alpha = 1,
		scale = 1.5
	} )
	self:registerAnimationState( "visible", {
	...
	local f25_local14 = f0_local0 and 0 or 10
	local self = LUI.UIText.new( {
		alignment = LUI.Alignment.Center,
		font = CoD.TextSettings.SP_HudCarbon27.Font,
		textStyle = CoD.TextStyle.Shadowed,
...
function splashesFanoutHudDef( f29_arg0 )
	...
	local f29_local5 = createCardSplashElement( "playerCardId" )
	f29_local5:registerOmnvarHandler( "ui_splash_playercard_idx", UpdateSplashPlayerCard )
	f29_local5:registerEventHandler( "splashes_hud_playercard_request", UpdateSplashPlayerCard )
	self:addElement( f29_local5 )
	...
	self:registerEventHandler( "playerstate_client_changed", function ( element, event )
		if not Game.InKillCam() and Game.GetPlayerClientnum() ~= Game.GetPlayerstateClientnum() then
			...
			if self.queue ~= self.ourQueue then
				...
				for f35_local1 = 1, f0_local7, 1 do
					f35_local0 = self.clipCache[f35_local1]
				...
				f35_local0.frame:cancelAnimateToState()
...
LUI.MenuBuilder.registerType( "splashesFanoutHudDef", splashesFanoutHudDef )
LockTable( _M )
