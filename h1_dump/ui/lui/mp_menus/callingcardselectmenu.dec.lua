...
function CallingCardSelectMenu( f1_arg0, f1_arg1 )
	...
	if IsPublicMatch() then
		local f1_local3 = "mp/callingCardNav.csv"
		local f1_local4 = 0
		while Engine.TableLookupByRow( f1_local3, f1_local4, 0 ) ~= "" do
			f1_local2.showNewSticker = LUI.InventoryUtils.GetSubCategoryNewStickerState( f1_local1, "cc_" .. Engine.TableLookupByRow( f1_local3, f1_local4, 3 ) )
	...
	f1_local0:AddButton( Engine.ToUpperCase( Engine.Localize( "@MENU_CLASSIC" ) ), "CallingCardMenu", nil, nil, nil, f1_local2 )
	if IsPublicMatch() then
		f1_local2.showNewSticker = LUI.InventoryUtils.GetSubCategoryNewStickerState( f1_local1, "cc_dlc" )
	end
	f1_local0:AddButton( Engine.Localize( "@MENU_DLC_CAPS" ), "CallingCardMenuDLC", nil, nil, nil, f1_local2 )
...
LUI.MenuBuilder.registerType( "CallingCardSelectMenu", CallingCardSelectMenu )
LockTable( _M )
